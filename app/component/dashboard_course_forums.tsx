...
export default function DashboardCourseForums({ openPost, courseId }: Props) {
  ...
  // One close handler per panel, so every exit path — the panel's own cancel
  // button, the backdrop, and Escape — clears exactly the same state. The
  // selection used to be reset in some paths and not others, which is how a
  // reply could open against the previously selected post.
        ...
        {replies.map((reply) => (
          <div key={reply.id} className="relative">
            ...
            <div className="relative pl-6 py-3">
              ...
              <p className="text-textGrey-0 text-sm mt-2 ml-10">{reply.content}</p>
    ...
    <>
      ...
      <div className="dashboard_content_mainbox">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-lightBoldText-0 dark:text-textSlightDark-0 font-bold text-[18px]">{t("Course Forum")}</h1>
          <button
      ...
      >
        <DashboardNewPost courseId={courseId} cancel={closePost} />
      </SlideOverModal>
