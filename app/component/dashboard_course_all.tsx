...
export default function DashboardCourseAllProvider({
  ...
  const setIsLoadingFunc = async (id: string) => {
    setIsLoading(true);
    setCourseId(id);
    await openCourse(id as any);
  };
        ...
        <div>
          ...
          {filterCourse.length === 0 ? (
            <div className="flex justify-center items-center flex-col gap-1 md:mt-10 mt-[8rem]">
              ...
              <h1 className="dark:text-textSlightDark-0 text-lightBoldText-0/80 font-semibold text-[18px]">
                No Course Found
              </h1>
            ...
            <div>
              {filterCourse.map((course, i) => (
                <div
                  key={i}
                  className="md:bg-[#ffffff] dark:md:bg-secondaryColors-0 md:drop-shadow-sm w-full md:p-[24px] my-5 flex flex-col gap-2"
                >
