...
export default function DashboardTutorTabForum({ openPost, courseId }: Props) {
  ...
  const toggleNestedReplies = async (replyId: string, currentReply: Reply) => {
    ...
    try {
      ...
      if (res.ok && data.data) {
        ...
        console.log("Failed to fetch nested replies");
      }
    } catch (error) {
  ...
  const handleReplyUpdate = (newReply: Reply, parentReplyId?: string) => {
    ...
    if (parentReplyId) {
      ...
      setRepliesByPostId((prev) => {
        const updated = { ...prev };
        Object.keys(updated).forEach((postId) => {
          updated[postId] = addNestedReply(updated[postId]);
        });
        return updated;
      });
  ...
  // FIXED: Open reply modal with proper postId
  const openReplyModal = (postId: string, parentReplyId?: string) => {
    console.log("Opening reply modal:", { postId, parentReplyId });
  ...
  const NestedRepliesList = ({
    replies,
    depth = 0,
    currentPostId, // Pass the postId down
  }: {
    replies: Reply[];
    depth?: number;
    currentPostId: string; // Add this to pass postId to nested replies
  }) => {
              ...
              {expandedReplies.includes(reply.id) && reply.children && (
                <div className="mt-2 ml-6">
                  <NestedRepliesList
                    ...
                    depth={depth + 1}
                    currentPostId={currentPostId} // Pass down the postId
                  />
                </div>
              )}
            </div>
                  ...
                  <div className="mt-5 ml-12 pl-4 border-l-2 border-gray-100">
                    {repliesByPostId[post.id as string]?.length > 0 ? (
                      <NestedRepliesList
                        ...
                        depth={0}
                        currentPostId={post.id as string} // FIXED: Pass postId to nested replies
                      />
                    ) : (
                      !isLoadingReplies[post.id as string] && (
