...
export default function CourseStep2({ formData, setFormData }: Props) {
  ...
  // In CourseStep2 component, update handleVideoUpload
  const handleVideoUpload = async (
    ...
    if (file) {
      ...
      try {
                  ...
                  lessons: m.lessons.map((l) =>
                    l.id === lessonId
                      ? {
                          ...
                          videoFile: file,
                          duration: durationInSeconds, // Store duration
                        }
                      : l,
                  ),
                }
        ...
        // Still upload but with default duration
        const previewURL = URL.createObjectURL(file);
        setModules((prev) =>
          prev.map((m) =>
            m.id === moduleId
  ...
  const getVideoSource = (lesson: Lesson) => {
    if (lesson.video_preview) {
      return lesson.video_preview;
    }
    return lesson.lesson_video;
  };
                                  ...
                                  {form.name === "lesson_video" ? (
                                    <div className="flex flex-col gap-1 w-full">
                                      ...
                                      {!lesson.video_preview && !videoSource ? (
                                        ...
                                        <div className="relative w-full h-[200px] border border-[#D2D5DA]/20 overflow-hidden rounded-md">
                                          <video
                                            src={videoSource}
                                            controls
