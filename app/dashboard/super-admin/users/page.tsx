  ...
  const toggleSuspend = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.message || t("Failed to update user"));
      }
    } catch (err) {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-center">
        <h1 className="dashboard_h1">{t("All Users")}</h1>
        <span className="text-textGrey-0 text-[13px]">{users.length} {t("total")}</span>
      </div>
      <p className="text-textGrey-0 text-[13px] mb-4">{t("Every account on the platform.")}</p>

      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="flex-1">
          <DashboardSearch value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("Search by name or email...")} />
        </div>
        <select
          value={filterRole}
        ...
        >
          {roles.map((r) => (
            <option key={r} value={r}>{r === "all" ? t("All roles") : r.replace("_", " ")}</option>
          ))}
        </select>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-2">
          ...
          <p className="text-textGrey-0 text-sm">{t("No users found")}</p>
        </div>
      ) : (
          ...
          <table className="w-full min-w-[700px] border-collapse">
            <thead>
              <tr className="border-b border-[#ccc]/10 text-left">
                <th className="py-3 px-2 text-[11px] font-[600] text-textGrey-0 uppercase">{t("User")}</th>
                <th className="py-3 px-2 text-[11px] font-[600] text-textGrey-0 uppercase">{t("Role")}</th>
                ...
                >
                  ...
                  <td className="py-3 px-2">
                    <span className={`text-[11px] font-[600] px-2 py-1 rounded-full capitalize ${roleBadge(u.role)}`}>
                      {u.role === "goye_admin" ? t("Platform Admin") : u.role.replace("_", " ")}
                    </span>
                  </td>
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-full md:w-[420px] bg-white dark:bg-secondaryColors-0 z-50 shadow-2xl overflow-y-auto p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-[20px] font-[700] text-textSlightDark-0 dark:text-white">{t("User Details")}</h2>
              ...
              <>
                <div className="flex flex-col items-center gap-2 mb-5">
                  ...
                  <span className={`text-[11px] font-[600] px-2 py-1 rounded-full capitalize ${roleBadge(selected.role)}`}>
                    {selected.role === "goye_admin" ? t("Platform Admin") : selected.role.replace("_", " ")}
                  </span>
                </div>
                ...
                <div className="space-y-2 text-[13px] mb-5">
                  ...
                  <div className="flex items-center gap-2 text-textGrey-0">🕑 <span className="text-textSlightDark-0 dark:text-white">{t("Joined")} {formatDistanceToNow(new Date(selected.createdAt), { addSuffix: true })}</span></div>
                </div>
                ...
                >
                  {pendingSuspend ? <FaSpinner className="animate-spin inline" /> : selected.isSuspended ? t("Reactivate User") : t("Suspend User")}
                </button>

                <h4 className="text-[13px] font-[700] text-textSlightDark-0 dark:text-white mb-2">
                  {t("Enrollments")} ({selected.enrollments.length})
                </h4>
                {selected.enrollments.length === 0 ? (
                  <p className="text-[12px] text-textGrey-0 mb-4">{t("No enrollments yet")}</p>
                ) : (
                      ...
                      <div key={e.id} className="bg-shadyColor-0/60 dark:bg-shadyColor-0 rounded-lg p-2">
                        <div className="flex justify-between items-center">
                          ...
                          <span className={`text-[10px] px-2 py-0.5 rounded-full ${e.status === "COMPLETED" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300"}`}>
                            {e.status === "COMPLETED" ? t("Completed") : t("In progress")}
                          </span>
                        </div>
                        <span className="text-[11px] text-textGrey-0">{t("Enrolled")} {formatDistanceToNow(new Date(e.enrolledAt), { addSuffix: true })}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {selected.memberships.length > 0 && (
                  <>
                    <h4 className="text-[13px] font-[700] text-textSlightDark-0 dark:text-white mb-2">{t("Organizations")}</h4>
