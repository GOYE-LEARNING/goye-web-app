...
export default function AdminCommunity() {
  ...
  // In your TutorCommunity component, fix the fetchGroups function:
  const fetchGroups = async () => {
    ...
    try {
      ...
      // 1. data.data (array of groups)
      // 2. data.groups (array of groups)
      // 3. data (direct array)
  ...
  const deleteGroup = (groupId?: GroupData) => {
    if (groupId?.id) {
      setGroup((prev) => prev.filter((g) => g.id !== groupId?.id));
    }
  };
                ...
                {filterCourse.length == 0 ? (
                  ...
                  <div>
                    ...
                    {filterCourse.map((data, i) => (
                      <div
                        className="border border-[#D2D5DA] bg-[#ffffff] py-[20px] px-[16px] flex flex-col gap-1 cursor-pointer my-4"
                        key={i}
                        onClick={() => {
                          setShowCommunity(false);
        ...
        {showCommunityGroup && (
          <div>
            <TutorCommunityGroup
              ...
              onDeleteGroup={() => deleteGroup({ id: groupId } as any)}
              groupId={groupId}
              backToMainPage={bactToMainPage}
            />
          </div>
        )}
      </div>
