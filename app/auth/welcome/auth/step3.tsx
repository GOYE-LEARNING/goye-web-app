"use client";
import { FaArrowRight, FaCheck } from "react-icons/fa";
import { useI18n } from "@/app/context/I18nContext";

export default function Step3({ formData, setFormData }: any) {
  const { t } = useI18n();
  const selected = formData.level;

  const formComponent = [
    { label: "Beginner", label_p: "I want to start fresh with the basics", value: "Beginners" },
    { label: "Intermediate", label_p: "I want to deepen my understanding", value: "Intermediate" },
  ];

  const handleSelect = (value: string) => {
    setFormData({ ...formData, level: value });
  };

  return (
    <div>
      <h1 className="form_h1">{t("Your experience level?")}</h1>
      <p className="form-p my-5">{t("Helps us tailor your content perfectly.")}</p>

      <form className="flex gap-5 my-5 flex-col">
        {formComponent.map((form, i) => {
          const isActive = selected === form.value;
          return (
            <label
              key={i}
              htmlFor={form.value}
              onClick={() => handleSelect(form.value)}
              className={`w-full border dark:border-[#ccc]/10 border-[#ccc]/50 p-3 flex justify-between items-center cursor-pointer transition-all duration-200 ${
                isActive ? "border-primaryColors-0 bg-primaryColors-0/5" : ""
              }`}
            >
              <input id={form.value} type="radio" name="level" checked={isActive} value={form.value} onChange={() => handleSelect(form.value)} className="hidden" />
              <div>
                <h1 className={`font-semibold ${isActive ? "text-primaryColors-0" : "dark:text-white text-lightBoldText-0/60"}`}>{t(form.label)}</h1>
                <p className="form-p">{t(form.label_p)}</p>
              </div>
              <div>
                <span className="text-primaryColors-0">{isActive ? <FaCheck size={12} /> : <FaArrowRight size={12} />}</span>
              </div>
            </label>
          );
        })}
      </form>
    </div>
  );
}
